...
def normalization(n):
    n = n/n[2]
    b = np.ones([1,3])
    b[0,0] = n[0]/np.sqrt(n[0]**2 + n[1]**2 + n[2]**2)
    b[0,1] = n[1]/np.sqrt(n[0]**2 + n[1]**2 + n[2]**2)
    b[0,2] = n[2]/np.sqrt(n[0]**2 + n[1]**2 + n[2]**2)
    return b
Boundary_points = []
Boundary_points_1 = []
x_points = 20
y_points = 10
a_1 = np.array([0,0,0.5])
a_2 = np.array([0,0,-0.5])
b = np.array([[0.5, 0.5, 0], [0.5, -0.5, 0], [-0.5, -0.5, 0], [-0.5, 0.5, 0]])
#v1 = np.array(b_1 - a_1)
#v2 = np.array(b_2 - a_1)
#n = np.cross(v1, v2)
#n = normalization(n)

def cal_Boundary_points(y_1,x_1,x_2):
    temp = []
    for i in range(x_points):
        temp1 = x_1 + (x_2 - x_1) * i/x_points
        for j in range(y_points):
            temp2 = y_1 + (temp1 - y_1) * j/y_points
            temp.append(temp2.tolist())
    v1 = np.array(x_1 - y_1)
    v2 = np.array(x_2 - y_1)
    n = np.cross(v1, v2)
    n = normalization(n)
    return np.array(temp), np.ones(np.array(temp).shape)*n
Boundary_points_1 = None
normal_vector_1 = None
for i in range(4):
    temp, temp1 = cal_Boundary_points(a_1,b[i % 4],b[(i+1) % 4])
    if Boundary_points_1 is None:
        Boundary_points_1 = temp
    else:
        Boundary_points_1 = np.concatenate((Boundary_points_1, temp), axis = 0)
    if normal_vector_1 is None:
        normal_vector_1 = temp1
    else:
        normal_vector_1 = np.concatenate((normal_vector_1, temp1), axis = 0)
for i in range(4):
    temp, temp1 = cal_Boundary_points(a_2,b[i % 4],b[(i+1) % 4])
    temp1 = -temp1
    Boundary_points_1 = np.concatenate((Boundary_points_1, temp), axis = 0)
    normal_vector_1 = np.concatenate((normal_vector_1, temp1), axis = 0)
Boundary_points_1 = Boundary_points_1 + 0.5
#print(Boundary_points_1.shape)
